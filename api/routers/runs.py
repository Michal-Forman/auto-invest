...
def list_runs(limit: int = 50, status: Optional[str] = None) -> List[RunResponse]:
    """Return runs with optional status filter, most recent first."""
    runs: List[Run] = Run.get_all_runs(limit=limit, status=status)
...
def get_run(run_id: str) -> RunDetailResponse:
    ...
    orders: List[Order] = Order.get_orders_for_runs([run_id])
