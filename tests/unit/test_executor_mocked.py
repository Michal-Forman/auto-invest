...
class TestPlaceT212Order:
    ...
    def _t212_response(self, filled_qty: float, qty: float, order_id: str = "T212-1") -> Dict[str, Any]:
        return {
            "req": None,
            "res": {
    ...
    def test_submitted_when_unfilled(
        ...
        mock_t212.equity_order_place_market.return_value = self._t212_response(0, 2.0, "T212-2")
        order = executor._place_t212_order("VWCEd_EQ", 5000.0, 1.0, run_id)
        assert order.status == "SUBMITTED"
    ...
    def test_partially_filled(
        ...
        mock_t212.equity_order_place_market.return_value = self._t212_response(1.0, 2.0, "T212-3")
        order = executor._place_t212_order("VWCEd_EQ", 5000.0, 1.0, run_id)
...
class TestPlaceOrders:
    def test_btc_routed_to_coinmate(
        ...
        mock_btc = mocker.patch.object(Executor, "_place_btc_order", return_value=MagicMock())
    ...
    def test_t212_tickers_routed_to_t212(
        ...
        mock_t212_order = mocker.patch.object(Executor, "_place_t212_order", return_value=MagicMock())
