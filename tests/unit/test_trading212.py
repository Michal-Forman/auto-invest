...
class TestPing:
    def test_returns_true_on_200(self, mocker: MockerFixture) -> None:
        mock_get = mocker.patch("core.trading212.requests.get")
        mock_get.return_value.status_code = 200
        assert Trading212.ping() is True

    def test_returns_true_on_401(self, mocker: MockerFixture) -> None:
        mock_get = mocker.patch("core.trading212.requests.get")
    ...
    def test_returns_false_on_connection_error(self, mocker: MockerFixture) -> None:
        ...
        assert Trading212.ping() is False
    ...
    def test_uses_live_host_in_prod(self, mocker: MockerFixture) -> None:
        mock_get = mocker.patch("core.trading212.requests.get")
        mock_get.return_value.status_code = 200
        Trading212.ping(env="prod")
        url = mock_get.call_args[0][0]
        assert "live.trading212.com" in url

    def test_uses_demo_host_in_dev(self, mocker: MockerFixture) -> None:
        mock_get = mocker.patch("core.trading212.requests.get")
        mock_get.return_value.status_code = 200
        Trading212.ping(env="dev")
        url = mock_get.call_args[0][0]
        assert "demo.trading212.com" in url

    def test_sends_no_auth_header(self, mocker: MockerFixture) -> None:
        mock_get = mocker.patch("core.trading212.requests.get")
        mock_get.return_value.status_code = 200
        Trading212.ping()
        call_kwargs = mock_get.call_args[1]
        assert "headers" not in call_kwargs or "Authorization" not in (
