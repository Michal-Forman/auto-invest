...
def _mock_instruments(mocker: MockerFixture, ath: float = 150.0, current: float = 100.0) -> None:
...
class TestCashDistributionToOrders:
    def test_cash_distribution_to_order_amounts_are_consistent(
        ...
        _mock_exchange_methods(
            mocker, t212, coinmate, t212_pie_response, t212_order_place_response, coinmate_buy_response
        )
    ...
    def test_idempotency_key_generated_and_unique_per_order(
        ...
        assert all(len(k) == 64 for k in keys)  # SHA-256 hex digest
    ...
    def test_process_new_run_data_fields_correct(
        ...
        assert (run_update.successful_orders or 0) + (run_update.failed_orders or 0) == len(orders)
